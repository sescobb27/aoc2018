defmodule Day13 do
  ...
  defmodule Cart do
    ...
    def turn(%__MODULE__{facing: :up, turns: [:left | turns]}, :intersection),
      do: {:left, turns ++ [:left]}

    def turn(%__MODULE__{facing: :down, turns: [:left | turns]}, :intersection),
      do: {:right, turns ++ [:left]}

    def turn(%__MODULE__{facing: :left, turns: [:left | turns]}, :intersection),
      do: {:down, turns ++ [:left]}

    def turn(%__MODULE__{facing: :right, turns: [:left | turns]}, :intersection),
      do: {:up, turns ++ [:left]}

    def turn(%__MODULE__{facing: :up, turns: [:right | turns]}, :intersection),
      do: {:right, turns ++ [:right]}

    def turn(%__MODULE__{facing: :down, turns: [:right | turns]}, :intersection),
      do: {:left, turns ++ [:right]}

    def turn(%__MODULE__{facing: :left, turns: [:right | turns]}, :intersection),
      do: {:up, turns ++ [:right]}

    def turn(%__MODULE__{facing: :right, turns: [:right | turns]}, :intersection),
      do: {:down, turns ++ [:right]}

    def turn(%__MODULE__{facing: facing, turns: [:straight | turns]}, :intersection),
      do: {facing, turns ++ [:straight]}
  ...
  # Based on JEG2 Stream https://www.twitch.tv/videos/348790864
  ...
  defp remove_crash(state) do
    if crash?(state) do
      %__MODULE__{
        state
        | moved:
            Enum.reject(state.moved, fn cart -> cart.location == hd(state.moved).location end),
          carts:
            Enum.reject(state.carts, fn cart -> cart.location == hd(state.moved).location end)
      }
    else
      state
    end
...
# r Day13; :aoc2018 |> :code.priv_dir() |> Path.join("day13.txt") |> Day13.part1()
# r Day13; :aoc2018 |> :code.priv_dir() |> Path.join("day13.txt") |> Day13.part2()
